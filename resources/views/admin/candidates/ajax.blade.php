                        ...
                        <table class="table table-bordered  candidatesTable " >
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" style="position:sticky; top:60px">Sub-Client</th>
                                            ...
                                            <tr data-id="{{ base64_encode($item->candidate_id) }}" candidate-d_id="{{ base64_encode($item->candidate_id) }}">
                                                ...
                                                <td>
                                                    ...
                                                    @if ($user_type == 'customer' || $cam!==null)
                                                        ...
                                                        @if($item->jaf_status =='filled')
                                                            ...
                                                            @if ($item->is_qc_done==0)
                                                                @if($hold) 
                                                                    <small> <span data-can_id="{{ base64_encode($item->candidate_id)}}">On Hold</span>  </small>
                                                                    <small> <span class="badge badge-success d-none" data-cand_id="{{ base64_encode($item->candidate_id)}}"> <a style="color:#fff;" href="{{ url('/candidates/jaf-qc',['id'=> base64_encode($item->candidate_id)]) }}"> BGV QC </a></span></small><br>
                                                                ...
                                                                @else
                                                                    <small>  <span class="d-none" data-can_id="{{ base64_encode($item->candidate_id)}}">On Hold</span></small>
                                                                    <small> <span class="badge badge-success" data-cand_id="{{ base64_encode($item->candidate_id)}}"> <a style="color:#fff;" href="{{ url('/candidates/jaf-qc',['id'=> base64_encode($item->candidate_id)]) }}">BGV QC</a></span></small><br>
                                                                    <a href="{{ url('/jaf-download',['id'=>base64_encode($item->candidate_id)]) }}" style="font-size: 10px;" class="btn-lnk" data-cand_id="{{ base64_encode($item->candidate_id)}}">Download BGV </a> <br>

                                                                @endif
                                                            @elseif ($item->is_qc_done==1 && $item->is_jaf_ready_report==0)
                                                                @if ($JAF_FILLED_ACCESS)
                                                                    ...
                                                                    @endif
                                                                @endif
                                                            @elseif ($item->is_qc_done==1 && $item->is_jaf_ready_report==1)
                                                                <!-- get report status -->
                                                                <?php $report_status = Helper::get_report_status($item->candidate_id); ?>
                                                ...
                                                <td>
                                                    ...
                                                    @if ($report_status==NULL || $report_status['status']=='incomplete' || $report_status['status']=='interim')
                                                        ...
                                                        @endif
                                                    @endif
                                                    <button class="btn btn-danger btn-sm closeCase mb-1" type="button" style="width: 40%;" data-id="{{ base64_encode($item->candidate_id) }}"> <i class='fa fa-crosshairs'></i> Close</button>
                                                    @if($item->jaf_status=='filled')
                                                        <?php $report_status = Helper::get_report_status($item->candidate_id); ?>
                                                        @if ($report_status==NULL || $report_status['status']=='incomplete')
