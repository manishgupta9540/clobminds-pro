...
<div class="modal" id="hold_resume_modal">
   ...
   <!-- close case Modal -->
<div class="modal" id="closed_case_modal">
    <div class="modal-dialog modal-lg">
       <div class="modal-content">
          <!-- Modal Header -->
          <div class="modal-header">
             <h4 class="modal-title" id="close_case_title">Closed Case </h4>
             <button type="button" class="close btn-disable" style="top: 12px;!important; color: red;" data-dismiss="modal"><small>×</small></button>
          </div>
          <!-- Modal body -->
             <div class="modal-body">
                   ...
                   <div class="col-12">
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            ...
                            <span class="candidate_comment"></span>
                        </div>
                    </div>
                    <div class="col-12">
                    <span class="candidate_file"></span>
...
$(document).ready(function(){
    ...
    $(document).on('click','.closedCase',function(event){
        var candidate_id = $(this).attr('data-id');
        
            $('#closed_case_modal').modal({
                backdrop: 'static',
                keyboard: false
            });
            ...
            $.ajax({
                type:'GET',
                url: "{{route('/candidates/closecasedata')}}",
                data: {'candidate_id':candidate_id},       
                success: function (data) {        
                    if(data !='null')
                    { 
                        // alert(data.result.additional_charge_notes);
                        //check if primary data candidate_comment
                        $('#closed_case_modal').find('.candidate_id').val(candidate_id);
                        $('#closed_case_modal').find('.business_id').val(business_id);
                        $('#closed_case_modal').find('.candidate_name').html(data.candidate_name);
                        $('#closed_case_modal').find('.candidate_comment').html(data.closed_case.close_comment);
                        $('#closed_case_modal').find('.candidate_file').html(data.attachfile);
