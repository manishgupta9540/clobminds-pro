               ...
               @if (count($tasks)>0)
                  @foreach ($tasks as $key=>$task) 
                     <tr>
                        ...
                        <?php 
                        // $diff= Helper::get_diff_days($task->candidate_id,$task->job_sla_item_id); 
                        ...
                        // $vendor_task = Helper::get_vendor_completed_task($task->id,$task->service_id,$task->number_of_verifications);
                          ...
                          $tat = Helper::get_diff($task->candidate_id);
                         $url = url('/').'/uploads/verification-file/'.$task->zip_file;
                        ?>
                        <td>
            ...
            @else 
               @if (count($tasks)>0)
                  @foreach ($tasks as $task)
                        ...
                        <td>
                           ...
                           @else
                              @if ($task->tastatus == '2')
                                  <span class="badge badge-info"> {{ Helper::user_name($task->assigned_to)}}</span>
                              @else
                                 <span class="badge badge-info"> {{ Helper::user_name($task->reassigned_to)}}</span>
                              @endif
                           @endif 
                        </td>
                        <td>
                     ...
                     @else
                        @if (($task->reassigned_to == '' && $task->assigned_to == Auth::user()->id  && ($task->tastatus =='2')) || ($task->reassigned_to == Auth::user()->id && ( $task->tastatus == '2' )) )
                           <tr>
                             <th scope="row"><input class="checks" type="checkbox" name="checks" value="{{ $task->id }}" onchange='checkChange();'></th>
