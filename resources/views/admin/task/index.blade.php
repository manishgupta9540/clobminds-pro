                     ...
                     <div class="col-md-2 form-group mb-1">
                      ...
                      <div class="col-md-2 form-group mb-1">
                        ...
                        <select class="form-control" name="task_type" id="task_type" >
                            ...
                            <option  value="Report QC">Report QC</option>
   ...
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <div class="modal-body">
            ...
            <div class="form-group">
               <input  type="radio" class="task_type" id="bgv_filling" name="task_type" value="bgv_filling" ><label for="bgv_filling"> BGV Filling</label>
               <input type="radio" class="task_type" id="bgv_qc" name="task_type" value="bgv_qc"><label for="bgv_qc"> BGV QC</label>
               <input type="radio" class="task_type" id="report_qc" name="task_type" value="report_qc"><label for="report_qc">Report QC</label>
               <p style="margin-bottom: 2px;" class="text-danger error-container" task_type="error-task_type"></p>
            </div> --}}
            <div class="form-group">
               <label for="label_name">Assign To </label>
               {{-- <div class="col-sm-12 col-md-12 col-12"> --}}
               <div class="form-group">
                  <select class="select-option-field-7 users selectValue form-control assign_user_list" id="filling_user" name="users" data-type="users" data-t="{{ csrf_token() }}">
                     <option value="">Select user</option>
                     {{-- @foreach ($users as $key => $user) 
...
<div class="modal" id="verify_assign_modal">
   <div class="modal-dialog">
      ...
      <!-- Modal Header -->
         <div class="modal-header">
            <h4 class="modal-title" id="assign_for" >Assign BGV</h4>
            {{-- <button type="button" class="close" data-dismiss="modal">&times;</button> --}}
         </div>
      <!-- Modal body --> 
...
<script type="text/javascript">
   ...
   $(document).ready(function(){
      ...
      //User get for BGV filling ,BGV qc nad Report QC
      $(document).on('change','.task_type',function(){
         var task_type=$('.task_type').val();

         $.ajax({
            type: 'GET',
            url:"{{ url('/task/assign/model') }}",
      ...
      // filterBtn
      $(document).on('change','.from_date, .to_date, .ref,.candidate_list,.sla_list,.user_list,#rows,#service,#task_type,#assign_status,.search,#complete_status', function (e){    
         $("#overlay").fadeIn(300);　
         getData(0);
         e.preventDefault();
      });
...
// });
         ...
         $('#task').modal({
            ...
            $.ajax({
               ...
               success: function (data) {
                  ...
                  if (data.fail == false ) {
                     $("#filling_user_name").append(" ");
                     
                     $('#filling_user_name').html('<option value="">Select user</option>');
                    
                        $("#filling_user_name").append(data.data);
                  }
               } 
   ...
   function getData(page){
      ...
      //   var report_status=$('.report_status').val();               
         ...
         $.ajax(
         {
               url: '?page=' + page+'&customer_id='+user_id+'&from_date='+from_date+'&search='+search+'&to_date='+to_date+'&candidate_id='+candidate_id+'&sla_id='+sla_id+'&user_id='+cus_user_id+'&rows='+rows+'&service_id='+service_id+'&task_type='+task_type+'&assign_status='+assign_status+'&complete_status='+complete_status+'&ref='+ref,
               type: "get",
               datatype: "html",
         })
         .done(function(data)
   ...
   function setData(){
      ...
      // var report_status=$('.report_status').val();
            $.ajax(
            {
               url: "{{ url('/') }}"+'/candidates/setData/?customer_id='+user_id+'&from_date='+from_date+'&to_date='+to_date+'&candidate_id='+candidate_id+'&sla_id='+sla_id+'&user_id='+cus_user_id+'&rows='+rows+'&service_id='+service_id+'&task_type='+task_type+'&assign_status='+assign_status+'&search='+search+'&complete_status='+complete_status+'&ref='+ref,
               type: "get",
               datatype: "html",
            })
            .done(function(data)
